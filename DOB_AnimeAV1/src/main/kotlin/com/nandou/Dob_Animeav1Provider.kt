...
class Animeav1DubProvider : MainAPI() {
    override var mainUrl = "https://animeav1.com"
    override var name = "DOB_AnimeAV1"
    ...
    override suspend fun load(url: String): LoadResponse {
        ...
        var firstEpUrl = "$mainUrl/ver/$slug/1"
        
        val firstHandshake = doc.selectFirst("a[href*=/media/], ul#eps li > a")
        if (firstHandshake != null) {
            firstEpUrl = fixUrl(firstHandshake.attr("href"))
        }

        try {
            val testDoc = app.get(firstEpUrl).document
            var hasDubbedContent = false
            testDoc.select("script").forEach { script ->
                if (script.data().contains("DUB", ignoreCase = true)) {
                    hasDubbedContent = true
                }
            }
            if (!hasDubbedContent) {
                throw Exception("No dubbed content available for this anime")
            }
        } catch (e: Exception) {
            throw Exception("Source filtered: Title does not have DUB tags")
        }
        ...
        return newAnimeLoadResponse(title, url, TvType.Anime) {
            this.posterUrl = fixUrl(poster)
            addEpisodes(DubStatus.Dubbed, if (sortedEpisodes.isNotEmpty()) sortedEpisodes else episodes)
